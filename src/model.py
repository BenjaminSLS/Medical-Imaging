...
def make_dataframe(lesions,lesions_not_biopsied):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=14) as executor:
        futures = [executor.submit(prepare_data_wrapper, class_instance) for class_instance in lesions]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=14) as executor:
        futures = [executor.submit(prepare_data_wrapper, class_instance) for class_instance in lesions_not_biopsied]
        for future in concurrent.futures.as_completed(futures):
            result_array.append(future.result())
...
def ohc(dataframe,ohc_features=[],drop_features=[]) :
    ...
    dummies = dummies.drop(["compactness","rotation-asymmetry","asymmetry","hue-sd"],axis=1)
...
def train_test_validate_split(X,y):
    ...
    X_test_train, X_test_test, y_test_train, y_test_test = train_test_split(X_test, y_test, test_size=0.5, random_state=42,shuffle=False)
